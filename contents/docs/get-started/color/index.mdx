---
title: Color
description: Moon Design System's colors are created with scalable and systemized approach, to be used across platforms.
---

## Scales

We create color palettes out of each core hue that is being used. This provides us with:
- **Consistent scales:** Scales maintain consistency with similar saturation and brightness levels. This ensures predictable and aesthetically pleasing color variations within the UI.
- **Structured theming:** Scales allow us to create separate palettes for different contexts (e.g. light/dark mode).

MDS Color system follows a scalable and systemized approach to be used across platforms:

- **Color scales per hue:** Each core color


are built using design tokens, represented as Figma variables. Color tokens are integrated into the entire design system, covering:
- Primitive tokens, consists of base hues and their palettes, forming the foundation of the system.
- Semantic tokens, mapped to specific UI roles, such as primary background or secondary text color.
- Component tokens, applied directly to specific UI components.
